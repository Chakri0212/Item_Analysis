WITH acts AS (
SELECT AF.source_system
	, AF.history_db_id
	, AF.student_id
	, AF.sequence_id AS activity_id	
FROM student_performance.activity_fact AF
JOIN kbs_billing.purchase_history PH
	ON PH.id = AF.kbs_enrollment_id	
	AND AF.date_completed BETWEEN {activity_start_date} AND {activity_end_date}
	AND (
		(AF.source_system = 'Atom' AND AF.sequence_name IN ({sequence_names}))
	)
	AND (AF.tutor_mode != 'True' OR AF.tutor_mode IS NULL)
	AND (AF.sequence_type NOT IN ('adaptive-qbank', 'qbank', 'test.qbank') OR AF.sequence_type IS NULL)
		
JOIN kbs_catalog.product P
	ON PH.product_id = P.ID

JOIN kbs_catalog.program PRG
	ON P.program_id = PRG.ID
	AND PRG.short_name IN ('NCLEX')
	

-- standard good faith removal of test accounts
JOIN (SELECT E_TEMP.*
		, ROW_NUMBER() over (PARTITION BY party_id ORDER BY list_index DESC) AS rowNumber
		FROM kbs_crm.email E_TEMP
		) email
	ON email.party_id = PH.person_id
	AND email.rowNumber = 1	
	
	-- I might need to do these with OR email.email is NOT NULL
	AND email.email NOT LIKE '%%@kaptest.com'
	AND email.email NOT LIKE '%%@kaplan.com'
	AND email.email NOT LIKE '%%@mailinator.com'
)
...
FROM atom_activity.section SEC
	...
	JOIN atom_activity.sequence SEQ
		ON SEC.sequence_id IN (SELECT activity_id FROM acts)
	...
	LEFT JOIN (
		...
		WHERE III_TEMP.sequence_id IN (SELECT activity_id FROM acts)
